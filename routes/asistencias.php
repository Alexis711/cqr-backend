...
$app->group('/asistencias', function ($app) {
    ...
    $app->get('/asistenciaporgrupo/{fechaInicio}/{fechaFin}/{grupo}', function ($request, $response, $args) {
        try {
            ...
            $sql = "SELECT asi.*, usu.nombres as nombresUsuario, usu.apellidos as apellidosUsuarios, eve.grupo as grupoEvento, eve.nombre as nombreEvento, eve.grupo as grupo, ubi.nombre as salon, eve.horaEntrada as horaEntradaEvento, eve.horaSalida as horaSalidaEvento FROM asistencias AS asi INNER JOIN usuarios AS usu ON asi.idUsuario = usu.idUsuario INNER JOIN eventos AS eve ON asi.idEvento = eve.idEvento INNER JOIN ubicaciones AS ubi ON eve.idUbicacion = ubi.idUbicacion INNER JOIN dias AS dia ON asi.idEvento = dia.idEvento WHERE asi.estatus != 0 AND (dia.$arrayDias[$diaSemana] = 1) AND (asi.fechaActual >= DATE('$fechaInicio') AND asi.fechaActual <= DATE('$fechaFin')) AND eve.grupo = '$grupo'";
